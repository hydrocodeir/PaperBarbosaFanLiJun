...
def role(p):
    n=p.stem
    if n.startswith('index_definition_'):return 'Controlled index/reference/window/quantile and in-base replacement sensitivity on 108 common stations (Figure 4, S14; Tables S12–S13). Conditional intervals; not independent confirmations.'
    if n.startswith('zero_threshold_'):return 'Structural-zero mixture identities, fixed observed strata and paired tie-definition effects (Figure 9, S15; Table S14); not independent robustness from exclusion.'
    if n.startswith('thermal_network_'):return 'Fixed-network thermal slopes, membership, synchronized bootstrap draws and paired contrast uncertainty (Table 1; S9–S11; Figures 2, S12–S13). Thresholds and station selection are held fixed.'
    if n == 'network_quantile_profiles_recomputed':return 'Historical available-network thermal profiles; retained as comparison evidence, not the primary fixed-network Figure 2.'
