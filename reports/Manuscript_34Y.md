...
Mean change alone is often insufficient for understanding thermal extremes because the most societally relevant impacts arise from changes in different parts of the distribution rather than from a uniform shift in its center. This study analyzes quantile-dependent trends in four annual thermal-extreme indices across Iran during 1991-2024 using observations from 124 meteorological stations. Warm days, warm nights, cool days, and cool nights were evaluated with station-wise quantile regression, moving-block bootstrap uncertainty estimation, false-discovery-rate field-significance screening, spatial diagnostics, and cluster-based regionalization. The results indicate a clear but non-uniform restructuring of thermal extremes. Warm days exhibit the strongest regional increase and the clearest upper-tail amplification, with regional network-mean slopes of `+9.45`, `+15.55`, and `+22.50` days per decade at `q0.10`, `q0.50`, and `q0.90`, respectively, and `Delta1 = +13.06`. Warm nights also increase, but more uniformly across the distribution, with corresponding slopes of `+8.74`, `+12.14`, and `+12.21` days per decade and `Delta1 = +3.47`. Cool days and cool nights decline throughout the country, with stronger upper-quantile contraction, especially for cool days (`-5.60`, `-12.00`, `-15.37`; `Delta1 = -9.77`) and, to a lesser extent, for cool nights (`-5.91`, `-9.83`, `-12.06`; `Delta1 = -6.16`). At `q0.50`, field-significance screening retained `115/124` warm-day, `104/124` warm-night, `97/124` cool-day, and `86/124` cool-night station results after FDR control. Moran's I was significant in `6/12` index-quantile fields, with the strongest and most consistent spatial organization in cool-day trends. Cluster-based regionalization likewise indicates dominant large clusters for warm nights and cool nights, but greater internal diversity for warm days and cool days. Sensitivity analyses show that the sign and broad ordering of the main results are robust to homogeneity-based station exclusion and deeper bootstrap resampling, whereas alternative bootstrap methods and some clustering specifications affect magnitude more than direction. Split-period comparisons suggest temporal nonlinearity rather than simple monotonic amplification across all quantiles. Overall, recent thermal-extreme change across Iran is distributionally asymmetric, spatially heterogeneous, and driven most strongly by upper-tail amplification of warm days and widespread contraction of cool extremes.
...
This study addresses that gap using a denser contemporary station network than is typical of long-record national syntheses. Using 124 meteorological stations for 1991-2024, we analyze annual warm days, warm nights, cool days, and cool nights with quantile regression, bootstrap uncertainty estimation, field-significance screening, spatial diagnostics, and cluster-based regionalization. The aim is not only to determine whether thermal extremes are changing, but also to establish how those changes are structured across the distribution, how consistently they organize in space, and how stable the resulting interpretation remains under alternative methodological choices.

Five main contributions frame the analysis. First, the study resolves changes across the conditional distribution rather than relying on mean change alone. Second, it measures distributional asymmetry with explicitly defined tail-contrast metrics. Third, it quantifies uncertainty through dependent bootstrap resampling and complementary significance diagnostics. Fourth, it regionalizes station behavior through reproducible clustering and representative-station selection. Fifth, it evaluates the stability of the main findings through homogeneity-exclusion, bootstrap-depth, bootstrap-method, interpolation, clustering, and split-period sensitivity analyses. Together, these elements support a distribution-aware climatological synthesis of recent thermal-extreme change across Iran.

## 2. Data and methods

### 2.1. Data and study region

The analysis is based on daily temperature observations from 124 meteorological stations distributed across Iran for the common period 1991-2024. After preprocessing, each station-index series contributes 34 annual values. The network spans coastal, lowland, plateau, and high-elevation settings and is dense enough to capture substantial physiographic heterogeneity while remaining fully observation-based.
...
The station-based design is deliberate. All primary inference is anchored in observed station records rather than interpolated grid cells, which is especially important in a region where elevation contrasts and marine influence can generate sharp local thermal differences over short distances. The daily archive includes minimum, maximum, and, where available, mean temperature. Leap-day observations were removed before percentile-threshold estimation to maintain a consistent 365-day annual cycle.

Data-quality screening showed generally high completeness across the network, with median station completeness of `98.96%` for `tmin`, `99.42%` for `tmax`, and `99.35%` for `tmean`. No station contained duplicate dates. Internal consistency diagnostics nevertheless identified at least one `tmin > tmax` conflict at 32 stations and at least one `tmean` value outside the daily temperature range at 78 stations. Screening therefore remained necessary even in a high-coverage archive. Homogeneity diagnostics applied to annual mean temperature were highly sensitive in raw form, flagging `119/124` stations, whereas detrended versions still flagged `39/124` stations. The contrast between raw and detrended tests indicates that breakpoint-sensitive homogeneity statistics respond strongly in a region with pervasive warming and should not be interpreted mechanically as evidence requiring direct daily-series correction.
...
No automatic homogenization adjustment was imposed on the daily temperature records. This choice was deliberate because the available archive does not include the station-history metadata or neighbor-based reference framework needed for a defensible daily homogenization procedure, and the main breakpoint diagnostics were applied to annual mean temperature rather than directly to the percentile-based daily series used for the indices (Zhang et al., 2005b). Accordingly, the study retains the original daily observations, treats the diagnostics as screening information, and uses flagged-station exclusion as a conservative sensitivity bound rather than as a substitute for full homogenization. Supporting summaries are provided in Supplementary Figure S1 and Supplementary Tables S1-S3, while the exclusion sensitivity is reported in Supplementary Figure S8 and Supplementary Table S4.

### 2.2. Thermal-extreme indices

Four annual thermal-extreme indices were constructed from daily minimum (`tmin`) and maximum (`tmax`) temperature: warm days, warm nights, cool days, and cool nights. For each station, day-of-year-specific thresholds were estimated from the available 1991-2024 record using a 5-day moving window and the 10th and 90th percentiles. A minimum of 15 reference samples per day-of-year window was required; when local window support fell below that threshold, the station-wide reference sample was used as the fallback.
...
Because only 1991-2024 data were available, the threshold climatology is based on the full analysis period rather than on a separate earlier reference interval. The resulting indices therefore describe how annual frequencies of warm and cool events evolved relative to the empirical distribution of the analyzed period itself. This produces internally consistent station-wise indices for the present dataset, but it also means that annual counts should be interpreted as distribution-relative diagnostics rather than as departures from a fixed pre-change climatological baseline.

### 2.3. Quantile regression and summary metrics

Quantile regression was used as the primary inferential tool because the goal was to resolve distribution-dependent change rather than only mean behavior (Koenker and Bassett, 1978; Koenker, 2005). For an annual response `Y` and time covariate `T`, the `tau`-level conditional quantile is written as
...
Bootstrap resampling was used to characterize uncertainty in the station-level quantile slopes. The baseline analysis used a moving-block bootstrap with 200 replicates. Because annual extreme-index series may retain short-range temporal dependence, simple iid resampling was not treated as the default approach. Block length was selected automatically with an `n^(1/3)` rule and then bounded to the interval `[2, 8]`, following the general dependent-bootstrap logic discussed by Kunsch (1989), Hall et al. (1995), and Lahiri (2003). For each focal quantile and for `Delta1`, we retained bootstrap means, standard deviations, and confidence intervals.

Inference in the tails relied primarily on the bootstrap summaries. This choice matters because the analytic tail outputs were insufficient to support a stable national FDR screening layer for the available dataset, whereas bootstrap intervals remained available for the focal tail summaries. Tail-significance counts are therefore discussed using the bootstrap results, while field-significance screening is reported for the median quantile, where the analytic station-level outputs were complete.
...
Several additional analyses were used to evaluate the stability and interpretability of the main results. First, a homogeneity-exclusion sensitivity analysis repeated the regional summaries after excluding all stations flagged by any detrended homogeneity diagnostic. Second, a bootstrap-depth sensitivity analysis repeated the bootstrap stage with 400 rather than 200 replicates. Third, a bootstrap-method comparison contrasted the baseline moving-block results against maximum-entropy bootstrap (`meboot`) summaries (Efron and Tibshirani, 1993; Vinod, 2006). Fourth, interpolation sensitivity compared alternative spatial-display methods so that mapped surfaces could be interpreted as visualization products rather than hidden inferential layers. Fifth, alternative clustering solutions examined sensitivity to linkage, distance metric, and algorithmic family. Sixth, split-period comparisons contrasted the first and second halves of the 1991-2024 record, namely `1991-2007` and `2008-2024`, using regional network-mean annual series. Finally, a simple driver analysis related slope and `Delta1` metrics to latitude, longitude, and elevation.

### 2.6. Analytical workflow

All analyses were implemented in a unified workflow that integrates data screening, annual-index construction, station-wise quantile regression, bootstrap uncertainty estimation, feature engineering, clustering, spatial diagnostics, and publication-oriented sensitivity modules. This workflow reduces manual transfer error between stages and ensures that all tables, figures, and regional summaries used in this study are traceable to the same underlying run configuration.
...
The station-level synthesis reinforces the regional profiles. `Delta1` is positive at `118/124` warm-day stations and `107/124` warm-night stations, but negative at all 124 cool-day stations and at `116/124` cool-night stations. Bootstrap tail inference also provides substantial support for these directional changes: significant bootstrap intervals occur at `q0.90` for 73 warm-day, 88 warm-night, 57 cool-day, and 61 cool-night stations, while `q0.10` significance occurs at 47, 69, 74, and 68 stations, respectively. Field-significance control at `q0.50` further indicates that the regional signal is not driven by only a few stations: the median slope remains significant after FDR control at most stations for all four indices.
...
The main directional conclusions remain stable under multiple sensitivity checks, but the magnitude of some summaries is method-dependent. Excluding all 39 stations flagged by any detrended homogeneity diagnostic leaves 85 stations and changes the regional network-mean `Delta1` values only modestly: warm days shift from `+13.06` to `+13.05`, warm nights from `+3.47` to `+3.45`, cool days from `-9.77` to `-10.39`, and cool nights from `-6.16` to `-4.87`. Thus, the sign structure and overall ordering are not driven by the flagged subset alone, even though the exact magnitude of cool-index asymmetry remains somewhat sensitive to station exclusion.

Bootstrap-depth sensitivity is likewise reassuring. Increasing the moving-block bootstrap from 200 to 400 replicates yields mean regional differences of only `-0.12` to `+0.09` across the four primary bootstrap mean metrics, with median absolute station-level differences mostly between `0.10` and `0.30` and station-to-station correlations above `0.996` in every case. By contrast, bootstrap-method sensitivity is more substantial in the tails: the mean absolute difference between moving-block and `meboot` summaries is about `0.97-1.16` days per decade at `q0.10` and `1.68-3.34` at `q0.90`. This does not imply that the results are unstable in sign; rather, it indicates that tail-uncertainty magnitudes depend materially on bootstrap design. Interpolation comparisons likewise show that mapped surfaces are more reproducible for some methods than for others. For example, linear RBF and linear interpolation agree strongly, with surface correlations of `0.84-0.93`, whereas thin-plate spline solutions diverge more strongly from those alternatives. This supports treating the maps as spatial displays rather than as independent inferential products.

Regionalization robustness is uneven and therefore informative. In the reduced-feature rerun, agreement with the baseline clustering is very high for cool nights (`ARI = 0.94`), moderate for warm nights (`0.49`), and low for warm days (`0.15`) and cool days (`0.06`). However, the alternative-method comparison shows that warm-day regionalization remains reasonably reproducible across several clustering algorithms (`ARI = 0.63-0.98`); warm nights are highly stable under the average-cityblock alternative (`0.94`) but less so under complete linkage and k-means; cool days remain only moderately reproducible (`0.36-0.46`); and cool nights range from exact reproduction under the average-cityblock variant (`1.00`) to low agreement under other alternatives (`0.12-0.21`). Taken together, these checks show that clustering is most robust for the dominant cool-night pattern and least uniquely determined for cool days, while warm-day clustering is sensitive to feature selection but still retains substantial structure across alternative hierarchical specifications.
...
The more distinctive contribution of the present analysis is that it shows recent Iranian warming to be strongly distributionally asymmetric and, in this dataset, more clearly led by daytime warm extremes than by nighttime ones. That result is physically plausible in an arid and topographically complex setting where land-surface feedbacks, cloud variability, and radiative conditions can favor strong daytime amplification (Seneviratne et al., 2010; Sanchez-Lorenzo et al., 2017; Perkins-Kirkpatrick and Lewis, 2020). At the same time, the low-elevation concentration of strong warm-night amplification and strong cool-night contraction is consistent with the growing importance of nocturnal and humid-heat risk in southern and coastal southwest Asia (Pal and Eltahir, 2016; Raymond et al., 2020; Raymond et al., 2024).

The results also fit the broader quantile-regression literature. Earlier studies showed that quantile-specific temperature trends can reveal structures hidden by OLS or mean-only analysis (Barbosa et al., 2011; Fan, 2014). The present study extends that logic to a dense national station network in Iran and shows that the same principle applies strongly in a dryland setting: warm-day amplification, warm-night flattening, and uneven cool-extreme contraction would all be partially obscured by mean trends alone. The study therefore adds regional specificity to the wider literature by demonstrating that recent thermal-extreme change across Iran is not only warming in sign but also structurally differentiated in shape.
...
More broadly, the study underscores a methodological point of wider relevance to climatology. In arid, topographically complex, and spatially heterogeneous environments, mean-only analyses can obscure the most consequential parts of the thermal signal. A quantile-aware, station-based framework is better suited to reveal whether change is distribution-wide, upper-tail dominated, or regionally uneven. In that sense, the present framework offers a transferable template for other dryland and mountainous regions where national climate-change assessments might otherwise smooth over precisely the contrasts that matter most for extremes.
