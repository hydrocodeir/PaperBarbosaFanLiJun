...
The results support a clear central claim: **thermal-extreme change across Iran is not adequately described by a simple mean shift alone, and a quantile-based synthesis reveals important distributional asymmetries in warm and cool event trends**. Warm extremes increase and cool extremes decline, but the strongest changes often occur away from the centre of the distribution and vary sharply among indices, stations, and data-driven grouping patterns. The main value of the analysis is therefore not only to confirm directional warming, but to show where in the distribution the most pronounced changes occur and how that structure differs across space.
...
These representative stations clarify the logic of the grouping structure while remaining fully reproducible. Because each panel shows centroid-nearest observed members rather than manually curated examples, the figure should be read as a visualization of typical cluster geometry. Warm-day groupings still separate upper-tail amplification from weakening or flatter profiles; warm-night groupings separate broad-based nocturnal intensification from stronger upper-tail growth; and the cool indices continue to distinguish contraction-dominated patterns from mixed or locally positive profiles. These data-driven grouping patterns are descriptively useful rather than merely statistically convenient, but they should not be read as uniquely identified climate regimes; the value of the figure lies in showing reproducible contrasts in quantile-slope shape under a transparent selection rule that can be regenerated exactly from the pipeline.
...
Third, an explicit spatial-coherence validation of the final cluster assignments shows that these regionalization outputs should be interpreted as similarity groupings in quantile-trend space rather than as geographically compact climate regions. For `cool_days`, `cool_nights`, `warm_days`, and `warm_nights`, the observed mean within-cluster station distances were about 727, 763, 729, and 752 km, respectively, while permutation p-values for being more spatially compact than random were `0.432`, `0.968`, `0.464`, and `0.740`. In other words, none of the four clusterings exhibited unusually strong geographic compactness relative to random relabelling. This result is scientifically useful: it aligns with the weak Moran's I evidence and confirms that the cluster analysis is summarizing distributional resemblance across stations, not estimating contiguous spatial regimes or solving the spatial-dependence problem at the model stage.
...
From a physical standpoint, the warm-night patterns are consistent with greater nocturnal heat retention at lower elevations and in settings where radiative cooling may be reduced, but the present analysis does not directly identify the responsible mechanisms. A plausible first mechanism is circulation-linked subsidence and advection in the broader eastern Mediterranean and MENA sector, which favour clear skies, compressional warming, and persistence of hot conditions `(Pal and Eltahir, 2016; Francis and Fonseca, 2024)`. A second mechanism is land-surface desiccation: in arid and semi-arid transition zones, reduced soil moisture suppresses evaporative cooling and shifts more of the available energy into sensible heating, thereby preferentially amplifying daytime hot extremes and the upper tail of warm-day counts `(Seneviratne et al., 2010; Fischer and Schär, 2010)`. A third mechanism is radiative. Decreasing cloudiness over the Mediterranean and adjacent subtropical belt has been linked to increased surface shortwave input and drying-favouring circulation changes, including poleward expansion of the Hadley cell `(Sanchez-Lorenzo et al., 2017; Francis and Fonseca, 2024)`. In the Iranian context, these mechanisms would be expected to interact with topography and continentality: elevated and interior stations may be especially exposed to strong daytime sensible heating under dry, cloud-poor conditions, whereas lowland and coastal or low-elevation stations can experience stronger nocturnal heat retention through weaker nighttime cooling and, in the south, more humid air masses that limit overnight relief `(Ahmadalipour and Moradkhani, 2018; Raymond et al., 2020)`.

The cool-day and cool-night indices show the opposite direction of change, but again with stronger contraction in upper quantiles, indicating that the most historically cool years are disappearing fastest. Mechanistically, that pattern is also consistent with a climate background in which circulation favouring clear, subsident, and dry conditions becomes more common, while reduced cloud-mediated cooling and stronger land-atmosphere coupling make unusually cool years harder to sustain. These interpretations should therefore be read as physically plausible explanations consistent with the statistical patterns, not as formal process attribution.
...
A second sensitivity check addressed bootstrap depth directly by increasing the number of resamples from 200 to 220 for all four indices (Supplementary Figure S11; Supplementary Tables S12-S13). The result is reassuringly stable for the main point summaries. Across `warm_days`, `warm_nights`, `cool_days`, and `cool_nights`, regional bootstrap means for `q0.05`, `q0.50`, `q0.95`, and `Delta1` changed by no more than about `0.75 days year^-1 per decade` in absolute mean value, and station-level correspondence for those mean summaries remained high to very high (`r = 0.74-1.00`, usually higher for the central and warm-night metrics). Confidence-interval widths were more sensitive than the means, as expected under a deeper resampling run, with widened intervals and more moderate station-level correspondence (`r` mostly about `0.41-0.87`). This additional check does not remove all tail uncertainty, but it does show that the manuscript's main sign, ordering, and asymmetry messages are not artefacts of the default 200-replicate configuration.

A third sensitivity check examined whether the regionalization itself depends strongly on the chosen clustering method and feature encoding (Supplementary Figure S12; Supplementary Tables S14-S15). The answer is mixed, and therefore informative. `warm_days` remained comparatively stable across alternative methods (`ARI = 0.63-0.92`), and `warm_nights` also retained a strong common structure for most alternatives (`ARI = 0.88-0.93`), although complete linkage produced a noticeably less similar partition (`ARI = 0.43`). `cool_nights` showed moderate method dependence, with one alternative reproducing the baseline almost exactly (`ARI = 1.00`) but others yielding only partial agreement (`ARI = 0.34-0.48`). `cool_days` were the least stable (`ARI = 0.05-0.35`), indicating that their grouping pattern should be read as exploratory rather than as a uniquely resolved partition. The additional spatial-coherence validation reaches a complementary conclusion: even where feature-space partitions are reasonably stable, they are not unusually geographically compact relative to random relabelling. This pattern does not invalidate the station-scale signal; rather, it shows that some grouping structures, especially for cooling-related indices, are weaker and less crisply separated than those for the warm indices, and that the clusters should be interpreted as descriptive data-driven grouping patterns rather than as sharply identified latent climate regimes.
...
11. `Supplementary Figure S13`: `outputs/figures/advanced_regionalization/cluster_spatial_validation.png`
12. `Supplementary Table S16`: `outputs/tables/regional_cluster_spatial_validation.csv`
