...
To strengthen the climate-change interpretation without overextending the study into formal attribution, we added a Quantile Climate-Fingerprint and Emergence Analysis (QCF-EA). This layer was designed to test whether the observed station trends form a consistent warming-related fingerprint across indices, quantiles, uncertainty estimates, and independent baseline choices. It is used here as observational signal detection, not as a counterfactual attribution experiment, and it is conceptually aligned with recent work on the emergence of climate-change signals in extreme-index distributions (Schuhen et al., 2026).
...
The conclusions should also be read in light of the limits of the current dataset. The record length is 34 years, the main thermal thresholds are derived from the available 1991-2024 period, and the fixed-baseline sensitivity uses only `1991-2007` as its early reference. The daily series were screened but not fully homogenized, Köppen-Geiger regimes simplify local climatic gradients, and some regionalization outcomes remain exploratory rather than uniquely resolved. Future work should therefore extend this analysis with metadata-supported daily homogenization, explicit spatial quantile models, reanalysis or model-based attribution comparisons, and physically richer diagnostics involving circulation, humidity, cloudiness, soil moisture, and surface energy balance. Compound heat metrics and event-based heatwave characteristics would also help connect the distributional changes identified here to human and environmental impact pathways. This step is especially relevant given recent evidence that global heat-stress impacts are increasingly shaped by exposure inequality and physiological survivability limits (Peng et al., 2026; Perkins-Kirkpatrick et al., 2026). Even with these caveats, the central implication remains consistent: recent thermal-extreme change across Iran is asymmetric, climatically differentiated, regionally structured, and best understood when tail behavior is treated as a primary component of climate inference rather than as a secondary descriptive detail.
