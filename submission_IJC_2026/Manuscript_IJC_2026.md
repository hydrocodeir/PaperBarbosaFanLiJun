...
**Authors:** [INSERT FULL AUTHOR LIST WITH AFFILIATION SUPERSCRIPTS AND A CORRESPONDING-AUTHOR ASTERISK]

**Affiliations:** [INSERT EACH DEPARTMENT, INSTITUTION, CITY, POSTAL CODE, AND COUNTRY]

**Author email addresses:** [INSERT AN EMAIL ADDRESS FOR EACH AUTHOR]

*Correspondence: [CORRESPONDING AUTHOR FULL NAME], [full postal address]. Email: [EMAIL]. ORCID: [ORCID].
...
The unresolved empirical question is whether distributionally uneven thermal-count changes coexist with increasing explicit dry–hot concurrence in this Iranian archive, and how the interpretation depends on marginal frequencies and zero precipitation across climate settings. The studies compared in Table S15 establish the individual approaches, but do not answer this combined question for the same archive. Recent probability decompositions (Zhao and Xiong, 2026) reinforce the need to state the added observational evidence precisely.
...
The archive contains 1,539,956 station-days without duplicate dates; median variable completeness ranges from 98.94% to 99.42%. Checks found 45 records at 32 stations with minimum temperature above maximum and a further 2,165 records at 61 stations with mean temperature outside that interval. Historical thermal estimates retain the supplied observations with a screened sensitivity. The compound analysis masks all temperatures for the first conflict and only mean temperature for the second; negative precipitation is treated as missing.
...
We evaluated six controlled constructions on the same common 108-station network, keeping raw temperature input, calendar, station selection and annual coverage unchanged (Table S12). The historical construction uses the full 1991–2024 reference, an 11-day window and linearly interpolated Hyndman–Fan type-7 quantiles (`numpy` method `linear`). A 5-day full-reference variant isolates window width. For the fixed 1991–2007 reference, we compare the uncorrected 11-day/type-7 series with corrected 11-day/type-7, corrected 5-day/type-7 and corrected 5-day/type-8 (`median_unbiased`) series. This sequence separates reference, in-base estimation, window and quantile-convention effects without searching all possible combinations.
...
Historical station-level moving-block and maximum-entropy bootstraps, FDR screening, Moran diagnostics, and clustering sensitivities are detailed in the Supporting Information (Künsch, 1989; Vinod, 2006; Benjamini and Hochberg, 1995). They are exploratory and distinct from primary fixed-network inference. Station maps show observed estimates without interpolation.
...
To separate sample composition from event definition, original balanced stations are classified by whether their observed baseline precipitation quartile is zero or positive, and these strata remain fixed in resampling. With strict inequalities and nonnegative precipitation, every zero-cutoff station has zero dry and joint frequency. Thus, for each network or climate group,
...
Configuration files and `python run_publication.py` reproduce the extension. Archived outputs include input hashes, package versions, membership, thresholds, bootstrap draws, tables, and vector figures. Separate validators rebuild indices and check membership, coverage, quantile fits, probability and dilution identities, contrasts, and intervals. The Supporting Information records the verification scope, including historical ensembles that were checked from saved draws rather than regenerated.
...
For each group we summarized station thermal slopes and fixed-baseline changes. Historical 999-label permutation tests compared the maximum-minus-minimum group mean across 32 prespecified index–metric combinations with FDR control. Spatial dependence limits exchangeability, so these omnibus results are supporting diagnostics rather than pairwise tests.
...
Station asymmetry maps retain the full 124-station archive and show substantial variation in point-estimate magnitude (Fig. 3); their station-level estimand differs from the fixed-network regressions in Section 3.1. Positive warm-day Δ₁ occurred at 118 stations and negative cool-day Δ₁ at all 124. The maps retain individual-station values; they do not imply a continuous trend surface. Historical Moran diagnostics gave the most consistent spatial organization for cool-day slopes, while clustering was sensitive enough to remain a supplementary description.
...
The unresolved-asymmetry conclusion was also retained: every Δ₁ sensitivity interval included zero (Fig. S14). For example, corrected 11-day/type-7 warm-day Δ₁ was 10.29 [−10.11, 22.69] days decade⁻¹, compared with 12.85 [−5.76, 19.66] for the historical full-record construction. Thus, the stable directional count change does not establish a precise invariant magnitude or a detected distributional asymmetry. Supplementary Figures S1–S8 retain the historical quality, spatial and clustering diagnostics with their original scope.

Historical available-network diagnostics were also directionally stable (Fig. S2). Excluding 39 detrended homogeneity flags changed Δ₁ by at most 1.29 days decade⁻¹, and masking inconsistent temperatures changed focal slopes by at most 0.026. These descriptive checks do not replace the primary fixed-network contrast intervals.

### 3.3. Climate regimes organize the spatial contrasts

All six climate groups had positive mean upper-quantile warm-day and warm-night slopes and negative mean cool-day and cool-night slopes (Fig. 5; Table 2). In these historical full-reference station estimates, the strongest mean upper-quantile warm-day trends occurred in BSh hot steppe (23.85 days decade⁻¹) and Dsa cold dry-summer settings (22.23), whereas BWh hot desert had a smaller warm-day slope (15.84) but the largest warm-night slope (16.64). Thus, the descriptive network-wide daytime point contrast did not describe every climate setting: hot-desert stations exhibited comparatively strong nighttime change.
...
[ACKNOWLEDGE THE IRAN METEOROLOGICAL ORGANIZATION AND ANY CONTRIBUTORS WHO DO NOT MEET AUTHORSHIP CRITERIA; OBTAIN THEIR PERMISSION BEFORE SUBMISSION.]

## Conflict of interest

[INSERT THE AUTHORS' APPROVED CONFLICT-OF-INTEREST STATEMENT; IF NONE: “The authors declare no conflicts of interest.”]

## Funding

[INSERT FUNDER NAMES AND GRANT NUMBERS; IF NONE: “This research received no external funding.”]

## Permission to reproduce material from other sources

[AUTHORS TO CONFIRM: No copyrighted material from other sources is reproduced. If any boundary data or other material requires permission or attribution, insert the required statement here.]

## Author contributions

[INSERT AN AUTHOR-BY-AUTHOR CRediT STATEMENT, FOR EXAMPLE: Conceptualization; Data curation; Formal analysis; Investigation; Methodology; Software; Validation; Visualization; Writing—original draft; Writing—review and editing.]

## Data availability statement

Daily station observations were supplied by the Iran Meteorological Organization (IRIMO). [AUTHORS TO CONFIRM ACCESS CONDITIONS, LICENSE, AND WHETHER THE OBSERVATIONS CAN BE REDISTRIBUTED.] Analysis code, configuration files, derived tables, figure-generation scripts, input hashes, and package versions are organized in the project repository. [INSERT A PUBLIC REPOSITORY URL AND PERSISTENT ARCHIVE IDENTIFIER (FOR EXAMPLE, A ZENODO DOI) BEFORE SUBMISSION.] If the source observations cannot be shared, state the formal request route to IRIMO and archive all legally shareable derived material and code.

## Supporting information

The accompanying [Supporting Information](Supporting_Information_IJC_2026.md) contains additional methods, complete component and sensitivity intervals, station-network diagnostics, controlled index-construction analyses, structural-zero and tie analyses, fifteen supporting figures, and fifteen supporting tables. Table S15 gives the detailed comparison with prior studies cited in the Discussion.
